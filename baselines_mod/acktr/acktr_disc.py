...
class Model(object):
    ...
    def __init__(self, policy, ob_space, ac_space, nenvs,total_timesteps, nprocs=32, nsteps=20,
        ...
        AX = tf.placeholder(tf.int32, [nbatch])
        AY = tf.placeholder(tf.int32, [nbatch])
        ...
        logpacx = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=train_model.pix, labels=AX)
        logpacy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=train_model.piy, labels=AY)
        self.logits = logits = train_model.pi
        self.logitsx = logitsx = train_model.pix
        self.logitsy = logitsy = train_model.piy

        ##training loss
        pg_loss = tf.reduce_mean(ADV*logpac) + 0.5*tf.reduce_mean(ADV*logpacx) + 0.5*tf.reduce_mean(ADV*logpacy)
        entropy = tf.reduce_mean(cat_entropy(train_model.pi)) + 0.5*tf.reduce_mean(cat_entropy(train_model.pix)) + 0.5*tf.reduce_mean(cat_entropy(train_model.piy))
        ...
        self.pg_fisher = pg_fisher_loss = -tf.reduce_mean(logpac) - 0.5*tf.reduce_mean(logpacx) - 0.5*tf.reduce_mean(logpacy)
        ...
        def train(obs, states, rewards, masks, actions, actionxs, actionys, values):
            ...
            td_map = {train_model.X:obs, A:actions, AX: actionxs, AY:actionys, ADV:advs, R:rewards, PG_LR:cur_lr}
            if states != []:
...
class Runner(object):
    ...
    def run(self):
        mb_obs, mb_rewards, mb_actions,mb_actionxs, mb_actionys, mb_values, mb_dones = [],[],[],[],[],[],[]
        mb_states = self.states
        for n in range(self.nsteps):
            actions, actionxs, actionys, values, states = self.model.step(self.obs, self.states, self.dones)
            mb_obs.append(np.copy(self.obs))
            mb_actions.append(actions)
            mb_actionxs.append(actionxs)
            mb_actionys.append(actionys)
            mb_values.append(values)
            mb_dones.append(self.dones)

            coord_actions = list(zip(actions, actionxs, actionys))

            obs, rewards, dones, _ = self.env.step(coord_actions)
            self.states = states
            self.dones = dones
        ...
        mb_actionxs = np.asarray(mb_actionxs, dtype=np.int32).swapaxes(1, 0)
        mb_actionys = np.asarray(mb_actionys, dtype=np.int32).swapaxes(1, 0)
        ...
        mb_actionxs = mb_actionxs.flatten()
